...
Home_GUI::Home_GUI(const User&& user) : m_currentUser(user)
{
    setupUi(this);
...
void Home_GUI::setupUi(QMainWindow* Home_GUIClass)
{
    ...
    actionRecommended = new QAction(Home_GUIClass);
    actionRecommended->setObjectName("actionRecommended");
	connect(actionRecommended, &QAction::triggered, this, &Home_GUI::openRecommended);
    ...
    QString imageFile = "images/background_image.jpg";
    QString imagePath = QDir(basePath).relativeFilePath(imageFile);
    ...
    imageFile = "images/search_icon.png";
    imagePath = QDir(basePath).relativeFilePath(imageFile);
    icon.addFile(imagePath, QSize(), QIcon::Normal, QIcon::Off);
    ...
    menu->addAction(actionRecommended);
...
void Home_GUI::retranslateUi(QMainWindow* Home_GUIClass)
{
    ...
    actionRecommended->setText(QCoreApplication::translate("Home_GUIClass", "Recommended", nullptr));
    ...
    groupBox->setTitle(QCoreApplication::translate("Home_GUIClass", "", nullptr));
...
{
    this->hide();
    m_userPage = new User_Page_GUI(std::move(m_currentUser));
    m_userPage->show();
...
void Home_GUI::openRecommended()
{
    this->hide();
	m_recommendedPage = new Recommended_GUI(std::move(m_currentUser));
    m_recommendedPage->show();
    connect(m_recommendedPage->Back_to_homepage, &QPushButton::clicked, this, &Home_GUI::onBackToMenuClickedRecommended);
}
...
void Home_GUI::onSearchButtonClicked()
{
    ...
    {
        this->close();
        m_moviePage = new Movie_Page_GUI(std::move(m_currentUser), allMovies[0]);
        m_moviePage->showDetails();
        m_moviePage->show();
...
void Home_GUI::onBackToMenuClickedRecommended()
{
    m_recommendedPage->close();
    this->show();
}
